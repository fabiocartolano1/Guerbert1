                 ...
                 <b-row style="margin-bottom : 0.5rem;">
                   <b-col>
                    ...
                    <b-col v-if="el.photos[0]">
                        <span v-for="(p,i) in el.photos" :key="p.id"> <span v-if="i!=0"> , </span>
                            {{p}}</span>
                    </b-col>
  ...
  computed : {
    ...
    methods:{
        ...
        async takePhoto2 (el){
            ...
            this.photos.push(
                await this.savePicture(cameraPhoto,fileName)
            )
        },
        convertBlobToBase64(blob) { return new Promise(function (resolve, reject) {
    ...
    for ( var i = 0 ; i < this.etat.Pieces.length ; i++){
        var p = {
            ...
            elElec : [
                ...
                {id: this.ID(), nom: "Interrupteurs", nature: "", etat: "", photos : []}    
            ],
